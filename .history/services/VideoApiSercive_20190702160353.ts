...
// import models
import {GetUserResponse,PostUserResponse,PutUserResponse} from '~/models/Users.ts'
...
class UserApiSercive {
  ...
  postUser(params: FormData) {
    return httpClient
      .post<PostUserResponse>(endpoints.video.POST, params)
      .then(response => response.data.data)
  }
  ...
  putUser(id: number, params: PutVideoRequest) {
    return httpClient
      .put<PutUserResponse>(`${endpoints.video.PUT}/${id}`, params)
      .then(response => response.data.data)
  }
}
